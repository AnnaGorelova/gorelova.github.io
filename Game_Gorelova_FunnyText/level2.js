...
async function loadGameData() {
    try {
        ...
        if (!gameData.level2_words || !Array.isArray(gameData.level2_words)) {
            throw new Error('Некорректная структура данных для уровня 2');
        }
...
function initializeStage1() {
    const gapPositions = generateRandomGapPositions(5, level2Words.length);
    ...
    for (let i = 0; i < level2Words.length; i++) {
        if (gapPositions.includes(i)) {
            const gapNumber = gapPositions.indexOf(i) + 1;
            textWithGaps += `<span class="gap-number" data-gap="${gapNumber}" data-original="${level2Words[i]}">${gapNumber}</span> `;
            gaps.push({
                number: gapNumber,
                originalWord: level2Words[i],
                userAnswer: null,
                isCorrect: false
            });
        } else {
            textWithGaps += level2Words[i] + ' ';
        }
    }
...
function checkStage(stageNumber) {
    ...
    if (mainTextElement) {
        ...
        gapElements.forEach(gapElement => {
            ...
            if (gapIndex !== -1) {
                if (gaps[gapIndex].isCorrect) {
                    gapElement.classList.add('correct');
                    correctCount++;
                } else {
                    gapElement.classList.add('incorrect');
                    if (!gaps[gapIndex].userAnswer) {
                        gapElement.textContent = gaps[gapIndex].originalWord;
                        gapElement.title = `Правильный ответ`;
                    } else {
                        const correctWord = gaps[gapIndex].originalWord;
                        gapElement.title = `Правильно: ${correctWord}`;
                    }
                }
            }
        });
